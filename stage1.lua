...
background = display.newImage("assets/graphics/back4.png")
...
local pikeSheetData = require "pike1"
local pikeData = pikeSheetData.getSpriteSheetData()


-- Pike Man sprite sheet
local pikeSheet = sprite.newSpriteSheetFromData("pike1.png", pikeData)

local pikeSet = sprite.newSpriteSet(pikeSheet, 1, 9)

sprite.add (pikeSet, "pikewalk1", 1, 1, 600, 1)
sprite.add (pikeSet, "pikewalk2", 3, 2, 700, 1)
sprite.add (pikeSet, "pikeattack", 1, 1, 300, 1)
sprite.add (pikeSet, "pikedie", 5, 2, 1000, 1)
sprite.add (pikeSet, "pikedead", 6, 1, 300, 1)
...
local ogreSheetData = require "ogre4"
local ogreData = ogreSheetData.getSpriteSheetData()

local ogreSheet = sprite.newSpriteSheetFromData("ogre4.png", ogreData)

local ogreSet = sprite.newSpriteSet(ogreSheet, 1, 6)

sprite.add (ogreSet, "ogrewalk1", 1,2, 800, 1)
sprite.add (ogreSet, "ogrewalk2", 4,1, 600, 1)
sprite.add (ogreSet, "ogreattack1", 3,2, 900, 1)
sprite.add (ogreSet, "ogreattack2", 4,1, 900, 1)
sprite.add (ogreSet, "ogredie1", 5,2, 1000, 1)
...
if event.sprite.sequence == "ogreattack1" and event.sprite.currentFrame == 2 and event.phase == "end" then
...
-- Hound sprite sheet
local houndSheetData = require "hound2"
local houndData = houndSheetData.getSpriteSheetData()

local houndSheet = sprite.newSpriteSheetFromData("hound2.png", houndData)

local houndSet = sprite.newSpriteSet(houndSheet, 1, 3)
...
scoreText = display.newText(localscore, 0, 0, "HelveticaNeue", 20)
...
debug1Text = display.newText(localscore, 0, 0, "HelveticaNeue", 15)
debug1Text:setTextColor(255, 255, 255)
debug1Text.x = 300
debug1Text.y = 995
--gameLayer:insert(debug1Text)
debug2Text = display.newText(localscore, 0, 0, "HelveticaNeue", 15)
debug2Text:setTextColor(255, 255, 255)
debug2Text.x = 300
debug2Text.y = 9115
--gameLayer:insert(debug2Text)
debug3Text = display.newText(localscore, 0, 0, "HelveticaNeue", 15)
...
local function RunMonster1(monster) 
  ...
  --   monster.sprite.x <= hero.x then
    ...
    if monster.state == "alive" and math.random(1, 30) <= monster.ai.aggression and monster.sprite.animating == false then
        ...
        elseif monster.state == "alive" and monster.sprite.animating == false and math.random(1, 30) <= 5 then
    ...
    if (monster.state == "dead" and math.random(1, 100) <= monster.ai.respawn) or ( monster.state == "dead" and  monsterL.state == "dead" and localscore > 2) then
...
local function RunMonster2(monster) 
 --   if monster.state == "alive" and          
  --   monster.sprite.x >= hero.x-70 then
    --    monster.sprite.x = monster.sprite.x - 2
    if monster.state == "alive" and math.random(1, 30) <= monster.ai.aggression and monster.sprite.animating == false then
        monster.sprite.x = monster.sprite.x + 3
    ...
    if (monster.state == "dead" and math.random(1, 150) <= monster.ai.respawn and localscore > 2 ) or (monster.state == "dead" and monsterR.state == "dead" and localscore > 2) then
       if lkills > 2 then
         return
       end       
...
monster.sprite.x = monster.sprite.x + 0.8
end

       
    if monster.state == "dead" and math.random(1, 300) <= monster.ai.respawn and localscore > 8 and monsterL.state == "alive" then
...
if monster.sprite.x - 40 > hero.x and monster.state == "alive" then
monster.sprite.x = monster.sprite.x - 0.7
end

       
    if monster.state == "dead" and math.random(1, 300) <= monster.ai.respawn and localscore > 12 and monsterR.state == "alive" then
	...
	if gameIsActive then
		...
		for i = 1, #toRemove do
         ...
         if OnButton == 0 then
          ...
          debug1Text.text = monsterR.state
          debug2Text.text = hero.sequence
          ...
          if monsterR.state == "alive" and hero.xScale == 1 and hero.x + 75 >= monsterR.sprite.x and hero.sequence == "hero2" and hero.currentFrame == 2 then
            if monsterR.sprite.sequence == "pikewalk1" then
          ...
          if monsterL.state == "alive" and hero.xScale == -1 and hero.x - 85 <= monsterL.sprite.x and hero.sequence == "hero2" and hero.currentFrame == 2 then
            if monsterL.sprite.sequence == "ogreattack1" and monsterL.sprite.currentFrame == 2 then
          ...
          --- KILLED BY PIKE
          if monsterR.state == "alive" and heroAlive == true and hero.x + 52 >= monsterR.sprite.x and hero.sequence ~= "herodie" then
             hero.y = hero.y + 10
             audio.play(sounds.die1)
            ...
            --- KILLED BY OGRE
          if monsterL.state == "alive" and heroAlive == true and hero.x - 60 <= monsterL.sprite.x and hero.sequence ~= "herodie" then
             hero.y = hero.y + 10
             audio.play(sounds.die1)
             hero:prepare("herodie")
             hero:play("herodie")
          ...
          origx= 0
			...
			if localscore > 30 then
				monsterBug.state = "dead"
				monsterBug.sprite:pause()
				monsterHound.state = "dead"
				monsterHound.sprite:pause()
				gameIsActive = false
			  director:changeScene ("stage2")
			  return 
			end
